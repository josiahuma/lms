        ...
        <div class="max-w-7xl mx-auto px-4">
            ...
            <section class="bg-indigo-600 text-white py-20 rounded-lg mb-12">
                <div class="text-center px-4">
                    <h1 class="text-4xl font-bold mb-4">{{ $settings->hero_heading ?? 'Unlock Your Potential' }}</h1>
                    <p class="mb-6 text-lg">{{ $settings->hero_subheading ?? 'Learn at your own pace...' }}</p>
                    <a href="{{ $settings->hero_button_link ?? route('courses.index') }}"
                    class="bg-white text-indigo-600 px-6 py-3 rounded shadow hover:bg-gray-100">
                        {{ $settings->hero_button_text ?? 'Browse Courses' }}
                    </a>
